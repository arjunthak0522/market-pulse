...
function loadSessionFix(){if(document.querySelector('script[data-session-fix]'))return;const s=document.createElement('script');s.src=`session-fix.js?v=14`;s.defer=true;s.dataset.sessionFix='true';document.head.appendChild(s)}
window.addEventListener('DOMContentLoaded',()=>{loadSessionFix();setTimeout(()=>{mobileDefaults();observeDates()},250);setTimeout(mobileDefaults,1400)});window.addEventListener('resize',mobileDefaults);
})();
